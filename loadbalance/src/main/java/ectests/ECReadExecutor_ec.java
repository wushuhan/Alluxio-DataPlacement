...
public class ECReadExecutor_ec implements Callable{

    private static final Logger LOG = LoggerFactory.getLogger(ECReadExecutor_ec.class);
    ...
    public ECReadExecutor_ec() throws Exception{
...
//        num++;
    ...
    public int readFileStragglerMitigate(
            ...
            boolean skipDecoding, byte[] data, int mKValueForEc, int mNValueForEc,
            long readCnt, int circle, int filenum
    ) throws Exception {
...
//        for (int blockNum = 0; blockNum < numTasks; blockNum++) {//todo：修改读请求发送策略
    ...
    //        if(waitTimeMs > 0) sleep(waitTimeMs);
            readCnt++;
            this.readFileStragglerMitigate(readType, numAdditionalReads, alluxioURI, fileName, skipDecoding, data,
                    mKValueForEC, mNValueForEC, readCnt,  i, filenum);
...
//        new FileWriter(logDirPath + "/decodingLatency.txt", true).write("--------------------------");
        originTimeMs = CommonUtils.getCurrentMs();
            while (!fileNamequeue.isEmpty()){
                ECReadExecutor_ec ecReadExecutor = new ECReadExecutor_ec();
