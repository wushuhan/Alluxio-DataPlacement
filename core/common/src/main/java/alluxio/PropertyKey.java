...
public final class PropertyKey implements Comparable<PropertyKey> {
  ...
  public static final PropertyKey WORKER_EVICTOR_LRFU_ATTENUATION_FACTOR =
      new Builder(Name.WORKER_EVICTOR_LRFU_ATTENUATION_FACTOR)
          ...
          .build();
  //[sjz] 增加LIRS策略参数
  public static final PropertyKey WORKER_EVICTOR_LIRS_HIR_PERCENT =
          new Builder(Name.WORKER_EVICTOR_LIRS_HIR_PERCENT)
                  .setDefaultValue(0.25)
                  .setDescription("The percent of HIR cache on each tier. LIRS divides each tier into HIR cache and LIR cache.\n" +
                                          "  HIR cache stores blocks with long reuse distance and LIR cache stores blocks with short reuse distance.")
                  .setConsistencyCheckLevel(ConsistencyCheckLevel.WARN)
                  .setScope(Scope.WORKER)
                  .build();
