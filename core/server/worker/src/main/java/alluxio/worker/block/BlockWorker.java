...
public interface BlockWorker extends Worker, SessionCleanable {
    ...
    /**
     * [sjz]
     * This method only called when open the block in Alluxio, the move action is limited on upper
     * adjacent tier, for example from tier 1 to 0 or from tier 2 to 1. Which need to check
     * the validity of move action based on 0/1 knapsack problem with approximate solution. Throws an
     * {@link IllegalArgumentException} if the tierAlias is out of range of tiered storage.
     *
     * @param sessionId the id of the client
     * @param blockId   the id of the block to move
     * @throws BlockDoesNotExistException  if blockId cannot be found
     ...
     *                                     block
     */
    void moveBlockWithPreCheck(long sessionId, long blockId) throws
            BlockDoesNotExistException,
            BlockAlreadyExistsException,
            InvalidWorkerStateException,
            WorkerOutOfSpaceException,
