...
public class TieredBlockStore implements BlockStore {
    ...
    private BlockMetadataManagerView initManagerView;
    ...
    public BlockReader getBlockReader(long sessionId, long blockId, long lockId)
        ...
        LOG.debug(
                "getBlockReader: sessionId={}, blockId={}, lockId={}",
                sessionId,
                blockId,
                lockId
        );
    ...
    @Override
    public TempBlockMeta createBlock(
            long sessionId, long blockId, BlockStoreLocation location,
        ...
        if (RESERVER_ENABLED) {
            ...
            throw new WorkerOutOfSpaceException(
                    ExceptionMessage.NO_SPACE_FOR_BLOCK_ALLOCATION_RETRIES_EXCEEDED,
                    initialBlockSize,
                    location,
                    MAX_RETRIES,
                    blockId
    ...
    public void requestSpace(long sessionId, long blockId, long additionalBytes)
        ...
        if (RESERVER_ENABLED) {
            ...
            // other types of exception to indicate this case.
            throw new WorkerOutOfSpaceException(
                    ExceptionMessage.NO_SPACE_FOR_BLOCK_REQUEST_SPACE_RETRIES_EXCEEDED,
                    additionalBytes,
                    MAX_RETRIES,
                    blockId
            );
    ...
    public void moveBlock(
        ...
        if (RESERVER_ENABLED) {
            RetryPolicy retryPolicy = new TimeoutRetry(FREE_SPACE_TIMEOUT_MS, EVICTION_INTERVAL_MS);
            while (retryPolicy.attempt()) {
                MoveBlockResult result = moveBlockInternal(
                        sessionId,
                        blockId,
                        oldLocation,
                        newLocation
                ...
                if (result.getSuccess()) {
                    synchronized (mBlockStoreEventListeners) {
                        for (BlockStoreEventListener listener : mBlockStoreEventListeners) {
                            listener.onMoveBlockByClient(sessionId,
                                                         blockId,
                                                         result.getSrcLocation(),
    ...
    @Override
    public void moveBlockWithPreCheck(long sessionId, long blockId)
            throws
            BlockDoesNotExistException,
            BlockAlreadyExistsException,
            InvalidWorkerStateException,
            WorkerOutOfSpaceException,
            IOException {
        //[sjz] 如果上层有足够空间，移动到上层，如果上层没有足够空间，需判断，若满足条件迁移


        long lockId = lockBlock(sessionId, blockId);
        BlockStoreLocation newLocation = null;
        BlockMeta meta = null;
        BlockStoreLocation oldLocation;
        try {
            meta = getBlockMeta(sessionId, blockId, lockId);
            oldLocation = meta.getBlockLocation();
            int ordinalOfOldLoc = mStorageTierAssoc.getOrdinal(oldLocation.tierAlias());
            int ordinalOfNewLoc = ordinalOfOldLoc - 1;
            // 获得上层location
            newLocation = BlockStoreLocation.anyDirInTier(mStorageTierAssoc.getAlias(ordinalOfNewLoc));
            LOG.info("moveBlockWithPreCheck: sessionId={}, blockId={}, oldLocation={}, newLocation={}",
                    sessionId, blockId, oldLocation, newLocation);
        } finally {
            //解锁块Id
            unlockBlock(lockId);
        ...
        if(EvictorUtils.selectDirWithRequestedSpace(meta.getBlockSize(), newLocation, initManagerView) == null && !mEvictor.checkWithKnapsackProblem(blockId, newLocation)){
            // 没有足够的空间且移动不能增加总体收益,则迁移不合理
            return;
        }

        boolean isMyEvictor = mEvictor.getClass().getSimpleName().equals(AffinityEvictor.class.getSimpleName());
        //判断mEvictor是否是MyEvictor
        if (!isMyEvictor){
            simpleMoveBlock(sessionId, blockId, oldLocation, newLocation);
            //不是MyEvictor， 使用普通迁移策略
        } else {
//            porbMoveBlock(sessionId, blockId, oldLocation, newLocation);
            simpleMoveBlock(sessionId, blockId, oldLocation, newLocation);
            //是MyEvictor， 使用概率迁移策略
        }
    }

    private void porbMoveBlock(long sessionId, long blockId, BlockStoreLocation oldLocation, BlockStoreLocation newLocation)
            throws IOException, BlockDoesNotExistException, InvalidWorkerStateException, BlockAlreadyExistsException, WorkerOutOfSpaceException {
        double[] ioLoad = MyEvictorUtils.updateIOLoad();
        double[] storageLoad = MyEvictorUtils.updateStorageLoad();
        double prompt_prob = AffinityEvictor.calculateP(storageLoad[0], ioLoad[0], "P");
        double evict_prob = AffinityEvictor.calculateP(storageLoad[1], ioLoad[1], "E");

        RetryPolicy retryPolicy = new CountingRetry(MAX_RETRIES);
        while (retryPolicy.attempt()) {
            if (new Random().nextDouble() < prompt_prob) {
            ...
            if (new Random().nextDouble() < evict_prob) {

                freeSpaceInternal(sessionId,
                        Objects.requireNonNull(getUpdatedView().getBlockMeta(blockId)).getBlockSize(),
                        newLocation,
                        Mode.GUARANTEED);
            }
        }
    ...
    private void simpleMoveBlock(long sessionId, long blockId, BlockStoreLocation oldLocation, BlockStoreLocation newLocation)
            throws IOException, BlockDoesNotExistException, InvalidWorkerStateException, BlockAlreadyExistsException, WorkerOutOfSpaceException {
        RetryPolicy retryPolicy = new CountingRetry(MAX_RETRIES);
        while (retryPolicy.attempt()) {
            MoveBlockResult result = moveBlockInternal(sessionId, blockId, oldLocation, newLocation);
    ...
    public void removeBlock(long sessionId, long blockId, BlockStoreLocation location)
            throws InvalidWorkerStateException, BlockDoesNotExistException, IOException {
        LOG.debug(
                "removeBlock: sessionId={}, blockId={}, location={}",
                sessionId,
                blockId,
                location
        );
    ...
    private void checkTempBlockIdAvailable(long blockId) throws BlockAlreadyExistsException {
        ...
        if (mMetaManager.hasBlockMeta(blockId)) {
            throw new BlockAlreadyExistsException(
                    ExceptionMessage.TEMP_BLOCK_ID_COMMITTED,
                    blockId
            );
        }
    }
    ...
    private TempBlockMeta createBlockMetaInternal(
        ...
        try (LockResource r = new LockResource(mMetadataWriteLock)) {
            ...
            StorageDirView dirView =
                    mAllocator.allocateBlockWithView(
                            sessionId,
                            initialBlockSize,
                            location,
                            getUpdatedView()
            ...
            TempBlockMeta tempBlock = dirView.createTempBlockMeta(
                    sessionId,
                    blockId,
                    initialBlockSize
            );
     ...
     */
    private Pair<Boolean, BlockStoreLocation> requestSpaceInternal(
            long blockId,
     ...
     */
    private void freeSpaceInternal(
            long sessionId, long availableBytes, BlockStoreLocation location,
            Evictor.Mode mode
        ...
        for (Pair<Long, BlockStoreLocation> blockInfo : plan.toEvict()) {
            try {
                ...
                LOG.info(
                        "Failed to evict blockId {}, it could be already deleted",
                        blockInfo.getFirst()
                );
                continue;
            }
        ...
        for (int tierOrdinal = mStorageTierAssoc.size() - 1; tierOrdinal >= 0; --tierOrdinal) {
            ...
            for (BlockTransferInfo entry : toMove) {
                ...
                if (moveResult.getSuccess()) {
                    synchronized (mBlockStoreEventListeners) {
                        for (BlockStoreEventListener listener : mBlockStoreEventListeners) {
                            listener.onMoveBlockByWorker(sessionId,
                                                         blockId,
                                                         moveResult.getSrcLocation(),
                                                         newLocation
                            );
                        }
                    }
    ...
    private void removeBlockInternal(long sessionId, long blockId, BlockStoreLocation location)
        ...
        try {
            ...
            try (LockResource r = new LockResource(mMetadataReadLock)) {
                if (mMetaManager.hasTempBlockMeta(blockId)) {
                    throw new InvalidWorkerStateException(
                            ExceptionMessage.REMOVE_UNCOMMITTED_BLOCK,
                            blockId
                    );
                }
    ...
    private static class MoveBlockResult {
         ...
         */
        MoveBlockResult(
                boolean success, long blockSize, BlockStoreLocation srcLocation,
                BlockStoreLocation dstLocation
        ) {
            mSuccess = success;
