...
//    private static final String logDirPath = System.getProperty("user.dir") + "/eclogs";
    private static final String logDirPath = "/home/wsh/erasure_coding/alluxio-origin/python/TieredStoreLogs";
    ...
    public ADSMEvictor(BlockMetadataManagerView view, Allocator allocator) {
        ...
        try {
            mLog = new BufferedWriter(new FileWriter(logDirPath+"/evictionLog.txt"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for (StorageTierView tier : mManagerView.getTierViews()) {
            ...
            for (StorageDirView dir : tier.getDirViews()) {
                ...
                for (BlockMeta block : dir.getEvictableBlocks()) {
                    ...
                    mBlockIdToSizeMB.put(blockId, new Double(block.getBlockSize() / (1024 * 1024)));
                    mBlockIdToTierOrdinal.put(blockId, tierOrdinal);
                    mBlockIdToBFOfMAS.put(
                            blockId,
                            getBenefitFactor(
                                    (double) (block.getBlockSize() / (1024 * 1024)),
                                    "MAS"
                            )
                    );
                    ...
                    if (tier.getTierViewOrdinal() == 0) {
                        ...
                        idToBR.put(
                                blockId,
                                mBlockIdToBFOfMAS.get(blockId) * mBlockIdToPValue.get(blockId)
                        );
                    } else {
                        // 1或2层
                        idToBR.put(
                                blockId,
                                mBlockIdToBFOfSAH.get(blockId) * mBlockIdToPValue.get(blockId)
                        );
                    }
                }
            }
    ...
    private double getBenefitFactor(
            ...
            int BWOfUpper,
            int BWOfCur,
            double SeekLatencyOfUpper,
            double SeekLatencyOfCur
    ) {
        double result = (1.0 / BWOfCur - 1.0 / BWOfUpper) + 1 / blockSize * (SeekLatencyOfCur - SeekLatencyOfUpper);
        return result;
    }
...
//        }
                    ...
                    for (StorageDirView dir : tier.getDirViews()) {
                        for (BlockMeta block : dir.getEvictableBlocks()) {
                            if (block.getBlockId() == blockId) {
                                ...
                                mBlockIdToBFOfMAS.put(blockId, getBenefitFactor((double) (block.getBlockSize() / (1024 * 1024)) , "MAS"));
                                mBlockIdToBFOfSAH.put(blockId, getBenefitFactor((double) (block.getBlockSize() / (1024 * 1024)) , "SAH"));
                                ...
                                if (mBlockIdToTierOrdinal.get(blockId) == 0) {
                                    map.put(blockId, mBlockIdToPValue.get(blockId) * mBlockIdToBFOfMAS.get(blockId));
                                } else {
                                    map.put(blockId, mBlockIdToPValue.get(blockId) * mBlockIdToBFOfSAH.get(blockId));
                                }
                            }
                        }
                    }
    ...
    public boolean checkWithKnapsackProblem(long blockId, BlockStoreLocation location) {
        ...
        promoteBenefit = mBlockIdToSizeMB.get(blockId) * mBlockIdToPValue.get(blockId) * (tierOrdinalOfBlock == 1 ? mBlockIdToBFOfMAS.get(
        ...
        while (evictBenefit < promoteBenefit) {
            ...
            if (it.hasNext()) {
                ...
                if (map.containsKey(evictId)) {
                    evictBenefit += (double) map.get(evictId) * mBlockIdToSizeMB.get(evictId) * mBlockIdToPValue.get(evictId);
                }
            } else {
                return false;
            }
        ...
        synchronized (mBlockIdToLastUpdateTime) {
            ...
            if (tierOrdinal == 0) {
                ...
                for (Map.Entry<Long, Double> entry : map.entrySet()) {
                    ...
                    map.put(
                            blockId,
                            mBlockIdToPValue.get(blockId) * mBlockIdToBFOfMAS.get(blockId)
                    );
                }
            } else {
                // 1 or 2层
                for (Map.Entry<Long, Double> entry : map.entrySet()) {
                    ...
                    map.put(
                            blockId,
                            mBlockIdToPValue.get(blockId) * mBlockIdToBFOfSAH.get(blockId)
                    );
                }
            }
        }
    ...
    private void updateOnMove(
        ...
        synchronized (mBlockIdToLastUpdateTime) {
            ...
            if (oldTierOrdinal != newTierOrdinal) {
                ...
                if(newTierOrdinal == 0){
                    mTierToMap.get(newTierOrdinal).put(blockId, mBlockIdToPValue.get(blockId) * mBlockIdToBFOfMAS.get(blockId));
                }else {
                    mTierToMap.get(newTierOrdinal).put(blockId, mBlockIdToPValue.get(blockId) * mBlockIdToBFOfSAH.get(blockId));
                }
            }
        }
    }
